...
## probabilities for borealization events
...
files <- list.files("./output/preindustrial_borealization_outcomes")

for(i in 1:length(files)){
  
  # i <- 1
  
  temp <- read.csv(paste("./output/preindustrial_borealization_outcomes/", files[i], sep=""))
...
files <- list.files("./output/historical_borealization_outcomes")

for(i in 1:length(files)){
  
  # i <- 1
  
  temp <- read.csv(paste("./output/historical_borealization_outcomes/", files[i], sep=""))
...
form <-  bf(count | trials(N) + weights(model_weight, scale = TRUE) ~
                period + s(me(annual.anomaly.1yr, test), by = period, k = 6) +
                s(ersst.year, by = period, k = 6) + (1 | model_fac))

far_brms_test <- brm(form,
                 data = prop,
