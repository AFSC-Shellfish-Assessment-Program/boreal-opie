...
sc_catch %>%
  ...
  filter(GIS_STATION %in% standard) -> cpue_large
...
cpue_large %>%
  ...
  pull(GIS_STATION) -> keepers_large 

#Final dataset for imputation
cpue_large %>%
  filter(GIS_STATION %in% keepers_large) -> dat_large
...
male_60_95_drop_5_map <- ggplot(plot_map, aes(longitude, latitude, color = mean_log_cpue)) +
...
dat_large <- tapply(dat_large$log_cpue, list(dat_large$YEAR, dat_large$GIS_STATION), mean)

# examine correlations
r <- rcorr(as.matrix(dat_large))$r 
...
colnames(pred) <- rownames(pred) <- colnames(dat_large) <- str_remove_all(colnames(pred), "-")

blocks <- mice::make.blocks(dat_large)

imp <- mice::mice(data = dat_large, method = "pmm", m=100, predictorMatrix = pred, blocks = blocks)

saveRDS(imp, "./output/abundance_imputations_male_60-95_stratum_drop_5th.RDS")
imp_60_95_drop_5 <- readRDS("./output/abundance_imputations_male_60-95_stratum_drop_5th.RDS")
...
# process - get mean and sd across imputations
...
lower_bound <- 0
upper_bound <- max(dat_small, na.rm = T) 
...
# plot.dat <- data.frame(year = c(1975:2019, 2021:2022),
#                        mean = tapply(imputed.back.trans.dat$mean, imputed.back.trans.dat$year, mean),
#                        sd = tapply(imputed.back.trans.dat$mean, imputed.back.trans.dat$year, sd))
...
# simpler .csv for now!
write.csv(plot.dat, "./output/male_60-95_drop5_df_simple.csv")
...
cpue_large %>%
  ...
  #b) Remove stations < 5th CPUE percentile range 
  filter(station_mean > quantile(station_mean, 0.05)) %>% #16 additional stations dropped
  pull(GIS_STATION) -> keepers_small 

#Final dataset for imputation
cpue_large %>%
  filter(GIS_STATION %in% keepers) -> dat_large
