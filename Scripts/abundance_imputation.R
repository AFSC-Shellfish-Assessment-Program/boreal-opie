...
saveRDS(imp, "./output/abundance_imputations_60_95.RDS")
imp <- readRDS("./output/abundance_imputations_60_95.RDS")
...
male.60.95.core <- ggplot(plot.dat, aes(year, log_mean)) +
  ...
  geom_errorbar(aes(ymin = log_mean - 2*sd,
                    ymax = log_mean + 2*sd)) +
  ggtitle("Male 60-95mm, core area")

male.60.95.core



# compare to entire stratum


stratum_haul <- haul %>%
  filter(GIS_STATION %in% unique(sc_strata$STATION_ID))

stratum_catch <- stratum_haul %>%
  select(GIS_STATION, SURVEY_YEAR, MID_LONGITUDE, MID_LATITUDE) %>%
  rename(YEAR = SURVEY_YEAR) %>%
  left_join(., scratch)

# replace NA CPUE with 0
change <- is.na(stratum_catch$CPUE)
stratum_catch$CPUE[change] <- 0
...
# log transform CPUE
stratum_catch$log_cpue <- log(stratum_catch$CPUE + 1)

plot <- stratum_catch %>%
  group_by(YEAR) %>%
  summarise(mean_log_cpue = mean(log_cpue))

ggplot(plot, aes(YEAR, mean_log_cpue)) +
  geom_line() +
  geom_point() # same pattern as for core area

## mice for male 30 - 60 mm ---------------
...
saveRDS(imp, "./output/abundance_imputations_30_59.RDS")
imp <- readRDS("./output/abundance_imputations_30_59.RDS")
...
male.30.59.core <- ggplot(plot.dat, aes(year, log_mean)) +
  geom_line(data = raw.dat, aes(year, log_mean), color = "red") +
  geom_point(data = raw.dat, aes(year, log_mean), color = "red") + 
  geom_line() +
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = log_mean - 2*sd,
                    ymax = log_mean + 2*sd)) +
  ggtitle("Male 30-59mm, core area")

male.30.59.core


## same for 30 + mm immature females --------------
...
saveRDS(imp, "./output/abundance_imputations_immature_female.RDS")
imp <- readRDS("./output/abundance_imputations_immature_female.RDS")
...
female.core <- ggplot(plot.dat, aes(year, log_mean)) +
  geom_line(data = raw.dat, aes(year, log_mean), color = "red") +
  geom_point(data = raw.dat, aes(year, log_mean), color = "red") + 
  geom_line() +
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = log_mean - 2*sd,
                    ymax = log_mean + 2*sd)) +
  ggtitle("Immature female >= 30mm, core area")

female.core

## mice for all immature ---------------
...
saveRDS(imp, "./output/abundance_imputations_all_immature.RDS")
imp <- readRDS("./output/abundance_imputations_all_immature.RDS")
...
immature.core <- ggplot(plot.dat, aes(year, log_mean)) +
  geom_line(data = raw.dat, aes(year, log_mean), color = "red") +
  geom_point(data = raw.dat, aes(year, log_mean), color = "red") + 
  geom_line() +
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = log_mean - 2*sd,
                    ymax = log_mean + 2*sd)) +
  ggtitle("Total immature, core area")
...
png("./Figs/core_immature_imputation_plots.png", width = 8, height = 6, units = 'in', res = 300)

ggpubr::ggarrange(male.30.59.core,
                  female.immature.core,
                  male.60.95.core,
                  immature.core,
                  ncol = 2,
                  nrow = 2)
...
imp <- readRDS("./output/abundance_imputations_all_immature_stratum_drop_5.RDS")
...
imp <- readRDS("./output/abundance_imputations_all_immature_stratum_drop_2nd.RDS")
...
## male 30-59, whole stratum, drop 5th quantile -------------
...
imp <- readRDS("./output/abundance_imputations_male_30-59_stratum_drop_5th.RDS")
...
male_30_59_stratum_drop_2nd <- ggplot(plot.dat, aes(year, log_mean)) +
  geom_line(data = raw.dat, aes(year, log_mean), color = "red") +
  geom_point(data = raw.dat, aes(year, log_mean), color = "red") + 
  geom_line() +
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = log_mean - 2*sd,
                    ymax = log_mean + 2*sd)) +
  ggtitle("Male 30-59 mm, stratum > 2nd percentile")

male_30_59_stratum_drop_2nd
